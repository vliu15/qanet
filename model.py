...
class Model(object):
    def __init__(self, config, batch, word_mat=None, char_mat=None, elmo_batcher=None, trainable=True, opt=True, demo = False, graph = None):
        self.config = config
        self.demo = demo
        ...
        with self.graph.as_default():
            ...
            if self.demo:
                ...
                self.co = tf.placeholder(tf.int32, [None, config.test_para_limit + 2, config.cont_char_limit],"context_cont")
                self.qo = tf.placeholder(tf.int32, [None, config.test_ques_limit + 2, config.cont_char_limit],"question_cont")
                ...
                self.c, self.q, self.ch, self.qh, self.co, self.qo, self.y1, self.y2, self.qa_id = batch.get_next()
            ...
            if opt:
                N, CL, CCL = config.batch_size if not self.demo else 1, config.char_limit, config.cont_char_limit
                ...
                self.co = tf.slice(self.co, [0, 0, 0], [N, self.c_maxlen + 2, CCL])
                self.qo = tf.slice(self.qo, [0, 0, 0], [N, self.q_maxlen + 2, CCL])
            ...
            if trainable:
                self.lr = tf.minimum(config.learning_rate, config.learning_rate / tf.log(999.) * tf.log(tf.cast(self.global_step, tf.float32) + 1))
    ...
    def forward(self):
        ...
        with tf.variable_scope("Input_Embedding_Layer", reuse=tf.AUTO_REUSE):
            ...
            c_emb_op = self.bilm(self.co)
            q_emb_op = self.bilm(self.qo)

            co_emb = tf.nn.dropout(weight_layers("context", c_emb_op, l2_coef=0.001)["weighted_op"], 1.0 - self.dropout)
            qo_emb = tf.nn.dropout(weight_layers("question", q_emb_op, l2_coef=0.001)["weighted_op"], 1.0 - self.dropout)

            c_emb = tf.concat([c_emb, ch_emb, co_emb], axis=2)
            q_emb = tf.concat([q_emb, qh_emb, qo_emb], axis=2)
